...
const { safeName, publicUser, clientIp } = require('../utils/helpers');
...
router.post('/register', async (req, res, next) => {
  try {
    ...
    if (!uid || uid.length < 3) return res.status(400).json({ ok: false, error: 'Realname tối thiểu 3 ký tự' });
    const ref = db.ref('users/' + uid);
    const exists = await ref.get();
    if (exists.exists()) return res.status(409).json({ ok: false, error: 'Realname đã tồn tại' });
    const passwordHash = await bcrypt.hash(body.password, 12);
    const user = { realname: uid, name: body.name.trim(), passwordHash, balance: 0, role: 'user', createdAt: admin.database.ServerValue.TIMESTAMP, lastIp: clientIp(req) };
    await ref.set(user);
    res.json({ ok: true, token: sign(uid), user: publicUser(uid, user) });
  } catch (err) { next(err); }
});
router.post('/login', async (req, res, next) => {
  try {
    ...
    const patch = { lastLogin: admin.database.ServerValue.TIMESTAMP, lastIp: clientIp(req) };
